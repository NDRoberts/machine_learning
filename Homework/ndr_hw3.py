...
def build_database(target):
    ...
    for cl in os.listdir(target):
        ...
        for name in os.listdir(f"{target}{cl}/"):
            ...
            sml = cv2.resize(img, (IMG_SCALE, IMG_SCALE),
    ...
    for k in test_k:
        for l in l_mode:
            ...
            aprf1 = (metrics.accuracy_score(y_fold1, predicts1),
                     metrics.precision_score(y_fold1, predicts1,
                                             average='macro', labels=labels),
                     metrics.recall_score(y_fold1, predicts1,
                                          average='macro', labels=labels),
                     metrics.f1_score(y_fold1, predicts1,
                                      average='macro', labels=labels))
            ...
            aprf2 = (metrics.accuracy_score(y_fold2, predicts2),
                     metrics.precision_score(y_fold2, predicts2,
                                             average='macro', labels=labels),
                     metrics.recall_score(y_fold2, predicts2,
                                          average='macro', labels=labels),
                     metrics.f1_score(y_fold2, predicts2,
                                      average='macro', labels=labels))
            ...
            aprf3 = (metrics.accuracy_score(y_fold3, predicts3),
                     metrics.precision_score(y_fold3, predicts3,
                                             average='macro', labels=labels),
                     metrics.recall_score(y_fold3, predicts3,
                                          average='macro', labels=labels),
                     metrics.f1_score(y_fold3, predicts3,
                                      average='macro', labels=labels))
...
if __name__ == "__main__":
    ...
    the_X_train = np.vstack((split[0][0], split[1][0],
                             split[2][0], split[3][0]))
    the_y_train = np.vstack((split[0][1], split[1][1],
                             split[2][1], split[3][1])).ravel()
